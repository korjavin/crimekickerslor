---
aliases: [Tiebi, Tiebe, Teibi, Taibi, CK-004]
tags: [character, hero, crime-kickers, canon]
canon: true
power: Volumetric scaling (size-shifting)
color: mustard
hub_code: CK-004
---

# Tiebi

One of the four [[The Crime Kickers|Crime Kickers]]. The team's **size-shifter** —
the ultimate infiltrator and, when needed, the heavy. Per [[Origin & Concept]], one
of the four fragments of the original fractured cyborg.

> *"Two sizes too many."* — tagline (Hub)

> **Spelling note:** appears as *Tiebi*, *Tiebe*, *Teibi*, and *Taibi* across sources.
> See [[Naming Variants]]. This vault uses **Tiebi** as the primary spelling.

## Canon snapshot (registry)

- **Appearance:** can **transform and adapt his physical form** to external conditions.
- **Power:** **extreme change of body size.**
  - **Lower limit:** smaller than an ordinary **cockroach**.
  - **Upper limit:** larger than an adult **elephant**.
  - **Transition between states is instantaneous.**

## Appearance (detailed)

From `godot-test1/docs/characters/tiebi.md` (spelled *Tiebe* there):

- **Build:** ~175–180 cm at normal size, medium build, slightly lean but healthy,
  realistic human proportions.
- **Face:** ordinary human head, neutral and friendly, no facial hair, normal
  expressive eyes, medium skin tone.
- **Signature feature — the beret:** a **classic French beret**, slightly tilted,
  dark navy or black, snug and soft with a small nub at the top center. This is the
  detail that defines and distinguishes him.
- **Clothing:** simple fitted shirt/tunic in neutral tones (gray, white, muted
  earth); standard trousers; simple flat black or brown shoes.
- **Vibe:** deliberately minimalist and human-like — the beret does the work.

When shifting, he keeps his proportions and his color scheme; the game prototypes
render a "child-like" small form with a proportionally larger head.

## Powers

- **Volumetric scaling (size-shifting)** — the core canon power.
- **Micro form:** shrink smaller than a cockroach — slip through cracks, vents,
  keyholes (the "ultimate infiltrator").
- **Macro form:** grow larger than an elephant — crush enemies, create obstacles,
  lift heavy objects.
- **Stomp shockwave** (Hub) — a ground-pound effect in macro form.
- **Physics note (Hub):** *"Mass conserves; momentum does not"* — a deliberate quirk
  worth exploiting in stories.
- **Vulnerable to low ceilings** (Hub).

## Hub profile (source material)

From the [[Crime Kickers Hub]] (`data.ts`) — captured but **not yet canon**:

- **Code:** CK-004 · **Power name:** Volumetric scaling · **Monogram:** T · **SFX:** STOMP
- **Real name:** not given.
- **Bio:** *"Tiebe is between 4 inches and 30 feet, depending on her mood. Mass conserves; momentum does not."*
- **Stats (0–5):** Reach 5 · Speed 4 · Recon 2 · Stealth 5
- **Capabilities:** macro form (≤ 30 ft); micro form (≤ 4 in); stomp shockwave.
  **Vulnerable:** low ceilings.

## Appears in

- [[Crime Kickers Hub]] — CK-004; featured in *"Tiebe vs. The School Bus"*.
- [[Godot Endless Runner]] — playable size-shifter model.
- [[Shadow Kickers (2D Platformer)]] — Micro Shift (crawl-gap puzzles); Titan Shift
  planned but not yet implemented.
- [[Stealth Kickers (Tactical Stealth)]] — Shrink + Grow. Here he is described as
  *"once a normal human, transformed into a being with complete control over his
  size"* — a transformation origin that **conflicts with the cyborg-fragment canon**.

## Open questions

- **Pronouns:** the Hub uses **she/her**; the Godot and Stealth Kickers notes use
  **he/his**. Unresolved — see [[Open Questions & Contradictions]].
- **Transformation vs. cyborg origin:** the *Stealth Kickers* "once a normal human"
  backstory predates and conflicts with the canon origin.

## Related

- [[The Crime Kickers]] · [[Origin & Concept]]
- Teammates: [[Windman]] · [[Pho-boman]] · [[Primm]]
