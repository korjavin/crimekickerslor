---
aliases: [S01E11, Closed Loop, Episode 11, The Recall Is a Contract Dispute, Name Inheritance]
...
format: comic
panels: 5
featured: [Windman, Tiebi, Pho-boman]
---

# S01E11 — Closed Loop

> **Logline:** Three of them open a case file to find out who is hunting them, and learn two things: the corporation isn't hunting them — it's closing a ticket — and the corporation has already given them a name.

---

## Title

**"Closed Loop"** — a control-systems term for a process that feeds its own output back into itself as an input, self-regulating without external intervention. Here it names three things simultaneously: the corporate workflow that has always been self-executing (the performance notice triggered the recall; the recall triggers reintegration; the ticket closes); the moment the heroes' investigation closes the loop on what has been hunting them (they learn the shape of the system from inside it); and the formal closure the episode performs on the chase register — not a villain hunting them, but a system resolving a liability, paperwork feeding paperwork. The loop has been running since GD-C-4419 was filed. The heroes did not start it. They find the loop already closed around them.

---

## Continuity / placement

**Movement II.** Direct seam from [[S01E10 — Half-Life]]: E10 closed on the three remaining — Windman, Tiebi, Pho-boman — outside, Primm inside a steel-frame containment unit, the ticket updated to `ASSET RECOVERED 1/4 — REINTEGRATION SCHEDULED, PENDING SECONDARY ASSET RECOVERY`. Windman is carrying what the episode named as "the gap where the fragment-resonance used to be." The three have no word for what has happened to the fourth; they have an understanding that they need one. E11 opens immediately from that condition: the three, ungrouped, the gap still fresh, finding a way to understand what they are dealing with.

Primm does not appear in this issue. He is held. The reintegration clock is running but paused: the specification holds W-01 and P-03 for joint reintegration, and W-01 is not in custody. Primm is a held asset pending secondary recovery. This is in the file.

Precedes [[S01E12 — Thermal Gradient]], in which Pho-boman's invisibility becomes a tactical tool and the rescue gains a target and a clock.
...
**Image:** Night street, lower-density district, post-E10 register — the same city, the same dark palette, but quieter. No trucks. No drones visible at this angle. Three figures, loosely spaced, not arranged for anything in particular. The space between each of them is still large — strangers who have not moved closer than the situation required — but they are facing the same direction, which by now is the only grammar they share.

**Windman:** left, fan folded at his side. Bandaged face forward. His pressure ghost-lines are out, the ambient field-read running quietly — the map of the city's air around him, its moving masses and still ones. In the space where the fragment-resonance signal would register — the dormant low note that ran under every panel he and Primm shared since E08, and that E10 Panel 5 showed going to a blank when Primm moved into steel and out of range — there is the same blank E10 left him with. The dash in his field has never been a label; in E08 Panel 2 it marked a signal he had no category for, and the bracket below carries that same un-named slot. What has changed since E08 is not the bracket's shape but its meaning: the slot that once read as an uncategorized presence now reads as an absence, a coordinate that used to return something and now returns nothing. The field-read bracket, visible in the panel's ghost-line overlay, reads exactly as it did at E08's convergence:
...
**Pho-boman:** right, slightly behind. Broth resettled and warm, the reddish-orange glow the panel's only warmth. Noodle-brows in the focused configuration — not alarmed, not curious; the configuration of someone who has located a direction and is walking toward it at his own pace. The belly-dragon faces outward. His route through the street has, without announcement, avoided the shadow of a refrigerated delivery unit parked at the far curb.

No drones visible. The city is quiet. The gap in the air is not pointed at.

**Caption / dialogue:**
- CAPTION: *The three of them had been in the city since.*
- CAPTION (below): *The fourth was somewhere that was not the city.*
- CAPTION (below): *Tiebi still had his credentials.*

**Why it lands:** The opening panel establishes the E10 aftermath in the driest possible register without restating E10's events. "The four of them" has become "the three of them" and "somewhere that was not the city" and the narrator does not explain any further — the reader has just come from E10. The FIELD READ bracket — unchanged in shape from E08's convergence, the same un-named dash in the same slot — is the panel's emotional core, delivered as a field-measurement: the slot reads identically, but where it once marked an uncategorized presence it now marks an absence, which is worse. Windman is reading the same notation and a different fact. Tiebi's credential token named in the final caption sets the episode's mechanism before the visual has even shown the terminal: the episode turns on bureaucratic access, and bureaucratic access turns on a small object in a human hand.
...
- CAPTION (below): *The hold condition: W-01 not yet in custody.*
...
- **Where the name came from.** The case file's operational designation "CRIME KICKERS" was auto-generated in E09 from the incident log entry "kicking-type force (Unit T-04)" — Tiebi's STOMP. The team has been named after the one of them who has a human past. The heroes have no access to the incident log field. They have the label. The audience holds what the label is built from.
...
- **The un-make question is not resolved.** The case file names the reintegration procedure in the same institutional language as E09's specification. The three heroes have now read both the specification (what will happen) and the case file (why it is happening). Neither document answers the question E18 will put to the fragments directly: what does it feel like to stand at the edge of un-making. That question is reserved beyond E11 and reserved beyond E20.
